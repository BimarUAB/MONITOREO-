...
function familyTasks(data) { return `<section class="card"><div class="section-heading"><div><h2>Tareas y actividades</h2><p class="muted">Entrega archivos y consulta la revisión docente de ${esc(data.estudiante.nombres)}</p></div></div>${data.tareas.map((task) => { const calificaciones = [['SER', task.calificacion_ser], ['SABER', task.calificacion_saber], ['HACER', task.calificacion_hacer]].filter(([, valor]) => valor !== null && valor !== undefined).map(([nombre, valor]) => `${nombre} ${valor}`).join(' · '); const estadoLabel = task.revisada ? 'Revisada' : task.estado === 'completada' ? (task.es_tardia ? 'Entregada tarde' : 'Entregada') : 'Pendiente'; return `<div class="task-row interactive"><button class="task-check ${task.estado === 'completada' ? 'done' : ''}" data-action="mark-task" data-task="${task.tarea_id}" data-student="${data.estudiante.id}" data-state="${task.estado}">${task.estado === 'completada' ? '✓' : ''}</button><div><strong>${esc(task.titulo)}</strong><small>${esc(task.materia)} · ${esc(task.descripcion || '')}</small>${calificaciones ? `<p class="task-feedback">Nota: ${esc(calificaciones)}</p>` : ''}${task.revision_comentario ? `<p class="task-feedback">Comentario docente: ${esc(task.revision_comentario)}</p>` : ''}<label class="evidence-upload"><span>${task.tiene_archivo ? 'Reemplazar evidencia' : 'Adjuntar evidencia'}</span><input type="file" data-upload-evidence="${task.entrega_id}" accept=".pdf,.jpg,.jpeg,.png,.doc,.docx,.ppt,.pptx,.xls,.xlsx,.txt,.zip"></label></div><div class="task-meta"><span class="chip ${task.revisada || task.estado === 'completada' ? 'chip-sobresaliente' : 'chip-bueno'}">${estadoLabel}</span><span class="review-status ${task.revisada ? 'reviewed' : ''}">${task.revisada ? `✓ Revisada${task.revision_fecha ? ` el ${fecha(task.revision_fecha)}` : ''}` : 'Pendiente de revisión'}</span><time>${task.fecha_entrega ? fecha(task.fecha_entrega) : 'Sin fecha'}</time></div></div>`; }).join('') || empty('No hay tareas asignadas')}</section>`; }
  ...
  try {
    ...
    const renderGrades = (trimestre) => {
      ...
      const actualizarTotal = (row) => {
        ...
        const tieneNota = inputs.some((input) => input.value !== '');
        const total = inputs.reduce((sum, input) => sum + Number(input.value || 0), 0);
        row.querySelector('.row-total').textContent = tieneNota ? String(Math.round(total * 100) / 100) : '—';
      };
      root.querySelectorAll('tbody tr').forEach((row) => row.querySelectorAll('[data-grade]').forEach((input) => input.addEventListener('input', () => actualizarTotal(row))));
